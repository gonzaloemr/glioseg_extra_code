...
def validation_data_clean(original_data_dir: list[str | Path], reference_dir: str | Path, data_destination_dir: str | Path):
    ...
    data_clean_dir = data_destination_dir.joinpath("new_run_corrected")
    ...
    for patient_dir in original_data_dir: 
        for patient in patient_dir.iterdir(): 
            if patient.name in patients_to_process: 
                ...
                for sub_dir in patient.iterdir():
                    ...
                    if sub_dir.is_dir():
                        ...
                        if sub_dir.name == "3D_T1C" or sub_dir.name == "CET1":  
                            ...
                            if len(nifti_files) > 1:
                                ...
                                sa_segmentation_file = next((f for f in nifti_files if "segmentation_native_" in f), None)
                                scan_file = sa_segmentation_file.removeprefix("segmentation_native_")
                                scan_dir_origin = sub_dir.joinpath(scan_file)
                                scan_dir_destination = patient_clean_nifti_dir.joinpath("T1GD.nii.gz")

                                segmentation_file = patient.joinpath("segmentation_native_updated_by-vckeil_corrected.nii.gz")
                                if not segmentation_file.exists():
                                    segmentation_file = patient.joinpath("segmentation_native_updated_by-vckeil_updated_corrected.nii.gz")
                                    if not segmentation_file.exists():
                                        segmentation_file = patient.joinpath("segmentation_native_updated_by-vckeil.nii.gz")
                                ...
                                gt_dir_origin = patient.joinpath(segmentation_file)
                                gt_dir_destination = gt_clean_nifti_dir.joinpath("MASK.nii.gz")
...
if __name__ == "__main__":
    ...
    original_data_dir = [
        Path("/data/share/IMAGO/Rotterdam_project/patients/"),
        Path("/data/share/IMAGO/Rotterdam_project/re-check"),
    ]

    reference_dir = Path("/scratch/radv/share/glioseg/new_run/Patients/")
    data_destination_dir = Path("/scratch/radv/share/glioseg/")

    validation_data_clean(original_data_dir, reference_dir, data_destination_dir)
