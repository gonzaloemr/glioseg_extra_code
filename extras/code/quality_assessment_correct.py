...
def compute_confidence_score_single_output(input_dir, segmentation_dict, labels, output_dir):
    ...
    for patient in input_dir.iterdir():
        ...
        # SINGLE mean aggregation per patient
        # final_macro = np.mean(macro_scores.values())
        # final_micro = np.mean(micro_scores.values())
...
def compute_confidence_score(input_dir, segmentation_dict, labels, output_dir):
    ...
    # --- Save results ---
    results = pd.DataFrame(all_results)
    output_file = output_dir.joinpath("confidence_candidates_extended.csv")
    results.to_csv(output_file, index=False)
    ...
    output_file_mean = output_dir.joinpath("confidence_candidates_summary.csv")
...
if __name__ == "__main__":
    ...
    output_dir = Path("/scratch/radv/share/glioseg/new_run/Confidence_assessment_single_output_2")
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    confidence_score_before_relabel = compute_confidence_score_single_output(
        input_dir=input_dir,
        segmentation_dict=segmentation_files_dict_before_relabeling,
        labels=labels,
        output_dir=output_dir_before_relabeling,
    )
    confidence_score_after_relabel = compute_confidence_score_single_output(
        input_dir=input_dir,
    ...
    # confidence_score_before_relabel = pd.read_csv(output_dir_before_relabeling.joinpath("confidence_candidates_summary.csv"))
    # confidence_score_before_relabel = {"Confidence Summary": confidence_score_before_relabel}

    # confidence_score_after_relabel = pd.read_csv(output_dir_after_relabeling.joinpath("confidence_candidates_summary.csv"))
    # confidence_score_after_relabel = {"Confidence Summary": confidence_score_after_relabel}

    # multiclass_dice_score_before_relabel = pd.read_csv(output_dir_before_relabeling.joinpath("multiclass_dice_score.csv"))
    # multiclass_dice_score_after_relabel = pd.read_csv(output_dir_after_relabeling.joinpath("multiclass_dice_score.csv"))
